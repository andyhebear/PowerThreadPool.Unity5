...
namespace PowerThreadPool_Net20
{
    public partial class PowerPool
    {
        ...
        public Group Watch<TSource>(
            ...
            int maxRetryCount = 0,int retryIntervalMs=100,
            ...
            WorkOption workOption = new WorkOption( maxRetryCount,TimeSpan.FromMilliseconds( retryIntervalMs));
            ...
            onCollectionChanged = (s,e) => {
                ...
                if (source.GetCanWatch() == CanWatch.Allowed) {
                    ...
                    source.TryTakeRange(count,out items);
            ...
            // 手动触发一次处理，处理初始队列中的元素
            onCollectionChanged(null,new NotifyCollectionChangedEventArgs<TSource>(NotifyCollectionChangedAction.Reset,default(TSource)));
        ...
        public void StopWatching<TSource>(ConcurrentObservableCollection<TSource> source,Group group = null
            //,bool keepRunning = false
            ) {
            //StopWatchingCore(source, group, false, keepRunning);
            //keepRunning会导致不确定性，所有的都是 StopWatchingCore(source,group,true,false);
            StopWatchingCore(source,group,true,false);
        }
